...
void AI_Placement()  // code
{
	AI *p_AI = AI::GetInstance();
	if (p_AI->GetMyTeam() == TEAM_1) {
		Game::PlaceTank(TANK_HEAVY, 6, 1);    //Tank1
		Game::PlaceTank(TANK_MEDIUM, 5, 14); //tank2
		Game::PlaceTank(TANK_LIGHT, 1, 8);   //tank3
		Game::PlaceTank(TANK_HEAVY, 2, 13);
	}
	else if (p_AI->GetMyTeam() == TEAM_2) {
		Game::PlaceTank(TANK_LIGHT, 15, 20);
		Game::PlaceTank(TANK_MEDIUM, 16, 17);
		Game::PlaceTank(TANK_MEDIUM, 20, 13);
		Game::PlaceTank(TANK_HEAVY, 16, 18);
	}
}
...
void AI_Update()
{
	...
	for (int i = 0; i < NUMBER_OF_TANK; i++) {
		...
		if (tempTank->GetTeam() == TEAM_1) {
			if (tempTank->GetX() < 20 && tempTank->GetY() == 1) {
				Game::CommandTank(0, DIRECTION_RIGHT, true, true);
			}
			else if (tempTank->GetX() == 20 && tempTank->GetY() < 16)
				Game::CommandTank(0, DIRECTION_DOWN, true, true);
		}
		//tank2 (5,14)   ->  ID= 1
		if (tempTank->GetTeam() == TEAM_1) {
			if (tempTank->GetX() < 9 && tempTank->GetY() == 14) {
				Game::CommandTank(1, DIRECTION_RIGHT, true, true);
			}
			else if (tempTank->GetX() == 9 && tempTank->GetY() > 12)
				Game::CommandTank(1, DIRECTION_UP, true, true);
			else if (tempTank->GetX() < 13 && tempTank->GetY() == 12)
				Game::CommandTank(1, DIRECTION_RIGHT, true, true);
			else if (tempTank->GetX() == 13 && tempTank->GetY() < 14)
				Game::CommandTank(1, DIRECTION_DOWN, true, true);
			else if (tempTank->GetX() < 20 && tempTank->GetY() == 14)
				Game::CommandTank(1, DIRECTION_RIGHT, true, true);
			else if (tempTank->GetX() == 20 && tempTank->GetY() > 1)
				Game::CommandTank(1, DIRECTION_UP, true, true);
		}
		//tank 3(1,8)  
		if (tempTank->GetTeam() == TEAM_1) {
			if (tempTank->GetX() < 20 && tempTank->GetY() < 20)
				Game::CommandTank(2, DIRECTION_UP, false, true);
		}
		//tank 4(2,13
		if (tempTank->GetTeam() == TEAM_1) {
			if (tempTank->GetX() < 20 && tempTank->GetY() < 20)
				Game::CommandTank(3, DIRECTION_LEFT, false, true);
		}///////////////////////////////////////
		//            /////////////////////////
	   //   TEAM 2  //////////////////////////
	  ///////////////////////////////////////
		if (tempTank->GetTeam() == TEAM_2) {
			if (tempTank->GetX() >1 && tempTank->GetY() == 20) {
				Game::CommandTank(0, DIRECTION_LEFT , true, true);
			}
			else if (tempTank->GetX() == 1 && tempTank->GetY() ==20)
				Game::CommandTank(0, DIRECTION_UP, false, true);
		}

		if (tempTank->GetTeam() == TEAM_2) {
			if (tempTank->GetX() == 16 && tempTank->GetY() >1) {
				Game::CommandTank(1, DIRECTION_UP, true, true);
			}
			else if (tempTank->GetX() > 5 && tempTank ->GetY() ==1)
				Game::CommandTank(1, DIRECTION_LEFT, true, true);
			else if (tempTank->GetX() ==5 && tempTank->GetY() <10)
				Game::CommandTank(1, DIRECTION_DOWN , true, true);
			else if (tempTank->GetX() == 5 && tempTank->GetY() ==10)
				Game::CommandTank(1, DIRECTION_LEFT, false, true);

		}
		//tank3
		if (tempTank->GetTeam() == TEAM_2) {
			if (tempTank->GetX() < 20 && tempTank->GetY() < 20)
				Game::CommandTank(2, DIRECTION_DOWN, false, true);
		}
